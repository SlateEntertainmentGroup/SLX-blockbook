...
func init() {
  ...
  //Slate mainnet Address enconding magics
  ...
  //Slate testnet Address encoding magics
...
// NewSlateParser returns new SlateParser instance
func NewSlateParser(params *chaincfg.Params, c *btc.Configuration) *SlateParser {
	p := &SlateParser{
...
// GetChainParams contains network parameters for the main Slate network
...
func (p *SlateParser) ParseBlock(b []byte) (*bchain.Block, error) {
	r := bytes.NewReader(b)
	w := wire.MsgBlock{}
	h := wire.BlockHeader{}
	err := h.Deserialize(r)
  ...
  //confirm if this is needed for slate
	if (h.Version > 1) {
...
func (p *SlateParser) PackTx(tx *bchain.Tx, height uint32, blockTime int64) ([]byte, error) {
	return p.baseparser.PackTx(tx, height, blockTime)
}

// UnpackTx unpacks transaction from protobuf byte array
func (p *SlateParser) UnpackTx(buf []byte) (*bchain.Tx, uint32, error) {
	return p.baseparser.UnpackTx(buf)
}

// ParseTx parses byte array containing transaction and returns Tx struct
func (p *SlateParser) ParseTx(b []byte) (*bchain.Tx, error) {
	t := wire.MsgTx{}
	r := bytes.NewReader(b)
...
func (p *SlateParser) TxFromMsgTx(t *wire.MsgTx, parseAddresses bool) bchain.Tx {
...
func (p *SlateParser) ParseTxFromJson(msg json.RawMessage) (*bchain.Tx, error) {
	var tx bchain.Tx
...
func (p *SlateParser) outputScriptToAddresses(script []byte) ([]string, bool, error) {
...
func (p *SlateParser) GetAddrDescForUnknownInput(tx *bchain.Tx, input int) bchain.AddressDescriptor {
	if len(tx.Vin) > input {
